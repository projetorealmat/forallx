...
Isso é válido mesmo que os dois componentes não sejam sentenças atômicas. Se você pode provar $[(A \eor J) \eif V]$ e  $[(V \eif L) \eiff (F \eor N)]$, então você efetivamente provou
...
Note que \script{A}, \script{x} e \script{c} são todas metavariáveis. Isto é, elas representam qualquer FBF, variável ou constante. E quando escrevemos \script{A}\script{c}, a constante \script{c} pode ocorrer múltiplas vezes na FBF \script{A}.
...
\end{quote}
Neste parágrafo, introduzimos um nome para a coisa que é um $S$. Demos a ela um nome arbitrário ('Ishmael') para que pudéssemos raciocinar sobre ela e derivar algumas consequências de haver um $S$. Como 'Ishmael' é apenas um nome fictício introduzido para o propósito da prova e não uma constante genuína, não poderíamos mencioná-lo na conclusão. No entanto, poderíamos derivar uma sentença que não menciona Ishmael; a saber, $\exists x Tx$. Esta sentença segue das duas premissas.
...
Uma constante que é usada para representar seja o que for que satisfaça uma afirmação existencial é chamada de \define{constante testemunha} (ou \emph{proxy}). O raciocínio com a constante testemunha deve ocorrer totalmente dentro de uma subprova, e ela não pode ser uma constante que esteja fazendo trabalho em outro lugar da prova.
...
Para uma prova curta, você pode ser capaz de eliminar as premissas e introduzir a conclusão. Uma prova longa é formalmente apenas um número de provas curtas ligadas entre si, então você pode preencher a lacuna alternando entre trabalhar de trás para frente a partir da conclusão e de frente para trás a partir das premissas.
...
Algumas regras de substituição devem se tornar segunda natureza. Se você vir uma disjunção negada, por exemplo, você deveria imediatamente pensar na regra de De Morgan.
...
Há uma maneira formal de mostrar que uma sentença é um teorema: Prove-a. Para cada linha, podemos verificar se essa linha segue pela regra citada. Pode ser difícil produzir uma prova de vinte linhas, mas não é tão difícil verificar cada linha da prova e confirmar que é legítima--- e se cada linha da prova individualmente é legítima, então toda a prova é legítima. Mostrar que uma sentença é uma tautologia, no entanto, requer raciocinar em português sobre todos os modelos possíveis. Não há uma maneira formal de verificar se o raciocínio é sólido. Dada a escolha entre mostrar que uma sentença é um teorema e mostrar que é uma tautologia, seria mais fácil mostrar que é um teorema.
