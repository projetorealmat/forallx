...
Como a sentença \ref{not2} é obviamente relacionada à sentença \ref{not1}, não queremos introduzir outra letra de sentença. Em parte, a sentença significa ``não $B$''. Para simbolizá-la, precisamos de um símbolo para a negação lógica. Usaremos `\enot'. Assim, podemos traduzir ``não $B$'' por $\enot B$.
...
A sentença \ref{and3} pode ser parafraseada como `$A$ e $B$'. Para simbolizá-la completamente, precisamos de outro símbolo. Usaremos `\eand'. Traduzimos então `$A$ e $B$' como $A\eand B$. O conectivo lógico `\eand' é chamado de \define{conjunção}, e $A$ e $B$ são chamados de \define{componentes da conjunção}.
...
\factoidbox{
Uma sentença pode ser simbolizada como $\script{A}\eand\script{B}$ se puder ser parafraseada em português como `Tanto \script{A} quanto \script{B}.' Cada um dos componentes deve ser uma sentença.
}
...
A sentença \ref{and6} é um pouco mais complicada. A palavra `embora' estabelece um contraste entre a primeira parte da sentença e a segunda. Ainda assim, a sentença diz tanto que Barbara é energética quanto que ela não é atlética. Para fazer com que cada um dos componentes seja uma sentença atômica, precisamos substituir `ela' por `Barbara'.
...
A conjunção é \emph{simétrica}, porque podemos trocar os componentes sem mudar o valor de verdade da sentença. Quaisquer que sejam \script{A} e \script{B}, \script{A}\eand\script{B} é logicamente equivalente a \script{B}\eand\script{A}.
...
\item[\ex{or.xor}] Você toma sopa ou salada, mas não as duas.
\end{earg}
...
A sentença \ref{iff2} é importante porque tem uma construção diferente. Ela pode ser parafraseada como `Se a figura tem três lados, então é um triângulo.' Assim, pode ser traduzida como $S\eif T$.
...
Poderíamos viver sem um novo símbolo para o bicondicional. Como \ref{iff3} significa `$T \eif S$ e $S\eif T$', poderíamos traduzi-la como $(T \eif S)\eand(S\eif T)$. Precisaríamos de parênteses para indicar que $(T \eif S)$ e $(S\eif T)$ são componentes separados da conjunção; a expressão $T \eif S\eand S\eif T$ seria ambígua.
...
Definimos uma \define{expressão de LS} como qualquer sequência de símbolos de LS. Pegue quaisquer símbolos de LS e escreva-os em alguma ordem: você terá uma expressão.


\subsection{Fórmulas bem formadas}

Como qualquer sequência de símbolos é uma expressão, muitas expressões de LS serão simplesmente sem sentido. Uma expressão significativa é chamada de \define{fórmula bem formada} (FBF).

Obviamente, letras de sentença individuais como $A$ e $G_{13}$ serão FBFs. Podemos formar novas FBFs a partir delas usando os conectivos. Usando negação, obtemos $\enot A$ e $\enot G_{13}$. Usando conjunção, obtemos $A \eand G_{13}$, $G_{13} \eand A$, $A \eand A$ e $G_{13} \eand G_{13}$. Também poderíamos aplicar negação repetidamente e obter FBFs como $\enot \enot A$, ou aplicar negação junto com conjunção e obter FBFs como $\enot(A \eand G_{13})$ e $\enot(G_{13} \eand \enot G_{13})$. As combinações possíveis são infinitas, mesmo começando apenas com essas duas letras de sentença, e há infinitas letras de sentença. Portanto, não faz sentido tentar listar todas as FBFs.

Em vez disso, descreveremos o processo pelo qual as FBFs podem ser construídas. Considere a negação: dada qualquer FBF \script{A} de LS, $\enot\script{A}$ é uma FBF de LS. É importante notar que \script{A} aqui não é a letra de sentença $A$. Ela é uma variável que representa qualquer FBF. Observe que essa variável \script{A} não é um símbolo de LS, de modo que $\enot\script{A}$ não é uma expressão de LS. Em vez disso, é uma expressão da metalinguagem que nos permite falar sobre infinitas expressões de LS: todas as expressões que começam com o símbolo de negação. Como \script{A} faz parte da metalinguagem, é chamada de \emph{metavariável}.

Podemos dizer algo semelhante para cada um dos outros conectivos. Por exemplo, se \script{A} e \script{B} são FBFs de LS, então $(\script{A}\eand\script{B})$ é uma FBF de LS. Fornecendo cláusulas desse tipo para todos os conectivos, chegamos à seguinte definição formal de \define{fórmula bem formada de LS}:

\begin{enumerate}
\item Toda sentença atômica é uma FBF.
\item Se \script{A} é uma FBF, então $\enot\script{A}$ é uma FBF de LS.
...
\item Todas e somente as FBFs de LS podem ser geradas por aplicações dessas regras.
\end{enumerate}

Note que não podemos aplicar imediatamente essa definição para verificar se uma expressão qualquer é ou não uma FBF. Suponha que queiramos saber se $\enot \enot \enot D$ é uma FBF de LS. Olhando a segunda cláusula da definição, sabemos que $\enot \enot \enot D$ é uma FBF \emph{se} $\enot \enot D$ for uma FBF. Então precisamos perguntar se $\enot \enot D$ é uma FBF. De novo, pela segunda cláusula, $\enot \enot D$ é uma FBF \emph{se} $\enot D$ for. E, por sua vez, $\enot D$ é uma FBF \emph{se} $D$ for uma FBF. Agora, $D$ é uma letra de sentença, uma sentença atômica de LS, então sabemos pela primeira cláusula que $D$ é uma FBF. Assim, para uma fórmula composta como $\enot \enot \enot D$, precisamos aplicar a definição repetidamente. Eventualmente, chegamos às sentenças atômicas a partir das quais a FBF é construída.
...
O conectivo que você observa primeiro ao decompor uma sentença é chamado de \define{operador lógico principal} daquela sentença. Por exemplo: o operador lógico principal de $\enot (E \eor (F \eif G))$ é a negação, \enot. O operador lógico principal de $(\enot E \eor (F \eif G))$ é a disjunção, \eor.


\subsection{Sentenças}
Lembre-se de que uma sentença é uma expressão significativa que pode ser verdadeira ou falsa. Como as expressões significativas de LS são as FBFs e como toda FBF de LS é verdadeira ou falsa, a definição de sentença de LS coincide com a definição de FBF. Nem toda linguagem formal terá essa propriedade agradável. Na linguagem LQ, desenvolvida mais adiante no livro, há FBFs que não são sentenças.
...
Se tivéssemos dado uma definição diferente de FBF, poderíamos fazer com que tais abreviações fossem FBFs. Poderíamos ter escrito a regra 3 assim: ``Se \script{A}, \script{B}, $\ldots$ \script{Z} são FBFs, então $(\script{A}\eand\script{B}\eand\ldots\eand\script{Z})$ é uma FBF.'' Isso tornaria mais fácil traduzir algumas sentenças em português, mas teria o custo de tornar nossa linguagem formal mais complicada. Teríamos de carregar essa definição complexa quando desenvolvêssemos tabelas de verdade e o sistema de provas. Queremos uma linguagem lógica que seja \emph{simples do ponto de vista formal} e ainda assim permita traduzir bem a partir do português (ou do inglês). Adotar convenções notacionais é um meio-termo entre essas duas exigências.
...
\item Se Alice ou Bob for espião, então o código foi decifrado.
...
\problempart
\label{pr.wiffLS}
Para cada expressão a seguir: (a) Ela é uma FBF de LS? (b) Ela é uma sentença de LS, levando em conta as convenções notacionais?
\begin{earg}
\item $(A)$
...
\problempart
\begin{earg}
\item Existe alguma FBF de LS que não contenha letras de sentença? Por quê?
