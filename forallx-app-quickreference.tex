...
\addcontentsline{toc}{chapter}{Apêndice C\quad Referência Rápida}
\pagestyle{plain}
...
Não é o caso que o F é G. (escopo amplo)& $\enot\exists x(Fx \eand \forall y(Fy \eif x=y) \eand Gx)$\\
O $F$ é não-$G$. (escopo restrito) & $\exists x(Fx \eand \forall y(Fy \eif x=y) \eand \enot Gx)$
\end{tabular*}
\end{center}
...
\item[n] $\exists x_1\cdots\exists x_n\bigl(Fx_1 \eand\cdots\eand Fx_n \eand \bigwedge_{1\leq i<j\leq n}x_i\neq x_j\bigr)$
\end{ekey}

Nas fórmulas genéricas, $\bigwedge$ e $\bigvee$ representam, respectivamente, a conjunção e a disjunção de todas as fórmulas indicadas pelos índices.
...
\bigl[(Fx_1\eand \cdots \eand Fx_{n+1}) \eif \bigvee_{1\leq i<j\leq n+1}x_i=x_j\bigr]$
\end{ekey}
...
\item[n] $\exists x_1\cdots\exists x_n\bigl[Fx_1 \eand\cdots\eand Fx_n \eand \bigwedge_{1\leq i<j\leq n}x_i\neq x_j \eand\\
 \enot\exists y\bigl(Fy \eand \bigwedge_{1\leq i\leq n}y\neq x_i\bigr)\bigr]$
