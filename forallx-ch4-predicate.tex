...
Uma vez que usamos letras sentenciais separadas para $N$ e $E$, apagamos qualquer conexão entre as duas. Elas passam a ser apenas duas sentenças atômicas que podem ser verdadeiras ou falsas de maneira independente. Em português, é impossível que, ao mesmo tempo, ninguém esteja confuso e todo mundo esteja confuso. Como sentenças de LS, porém, existe uma atribuição de valores de verdade para a qual $N$ e $E$ são ambas verdadeiras.
...
A sentença \ref{q.ne} pode ser parafraseada como `Não é o caso que alguém está com raiva.' Isso pode ser traduzido usando negação e um quantificador existencial: $\enot \exists x Ax$. No entanto, a sentença \ref{q.ne} também pode ser parafraseada como `Ninguém está com raiva.' Com isso em mente, pode ser traduzida usando negação e um quantificador universal: $\forall x \enot Ax$. Ambas são traduções aceitáveis, porque são logicamente equivalentes. O ponto crítico é se a negação vem antes ou depois do quantificador.
...
\item[\ex{dog2}] Gerald é dono de um cachorro.
\item[\ex{dog3}] Alguém é dono de um cachorro.
\item[\ex{dog4}] Todos os amigos de Gerald são donos de um cachorro.
\item[\ex{dog5}] Todo dono de um cachorro é amigo de algum dono de um cachorro.
\end{earg}
...
A sentença \ref{dog3} pode ser parafraseada como: `Existe algum $y$ tal que $y$ é dono de um cachorro.' A subsentença `$y$ é dono de um cachorro' é exatamente como a sentença \ref{dog2}, exceto pelo fato de que é sobre $y$ em vez de ser sobre Gerald. Assim, podemos traduzir a sentença \ref{dog3} como $\exists y \exists x(Dx \eand Oyx)$.

A sentença \ref{dog4} pode ser parafraseada como: `Todo amigo de Gerald é dono de um cachorro.' Traduzindo parte dessa sentença, obtemos $\forall x(Fxg \eif\mbox{`$x$ é dono de um cachorro'})$. Mais uma vez, é importante reconhecer que `$x$ é dono de um cachorro' é estruturalmente igual à sentença \ref{dog2}. Como já temos um quantificador sobre $x$, precisaremos de uma variável diferente para o quantificador existencial. Qualquer outra variável serve. Usando $z$, a sentença \ref{dog4} pode ser traduzida como $\forall x\bigl[Fxg \eif\exists z(Dz \eand Oxz)\bigr]$.

A sentença \ref{dog5} pode ser parafraseada como `Para qualquer $x$ que é dono de um cachorro, existe um dono de um cachorro que é amigo de $x$.' Traduzida parcialmente, isso se torna
$$\forall x\bigl[\mbox{$x$ é dono de um cachorro}\eif\exists y(\mbox{$y$ é dono de um cachorro}\eand Fxy)\bigr].$$
...
Nesta seção, fornecemos uma definição formal de \define{fórmula bem formada} (FBF) e de \emph{sentença} de LQ.
...
Assim como fizemos em LS, daremos uma definição \emph{recursiva} para uma FBF de LQ. Na verdade, a maior parte da definição será muito parecida com a definição de FBF de LS: toda fórmula atômica é uma FBF, e podemos construir novas FBFs aplicando os conectivos sentenciais.

Poderíamos simplesmente acrescentar uma regra para cada um dos quantificadores e encerrar o assunto. Por exemplo: se \script{A} é uma FBF, então $\forall x\script{A}$ e $\exists x\script{A}$ são FBFs. No entanto, isso permitiria sentenças esquisitas como $\forall x\exists x Dx$ e $\forall x Dw$. O que essas fórmulas poderiam significar? Poderíamos adotar alguma interpretação para tais sentenças, mas, em vez disso, vamos escrever a definição de FBF de modo que tais aberrações nem mesmo contem como bem formadas.

Para que $\forall x\script{A}$ seja uma FBF, \script{A} deve conter a variável $x$ e não deve já conter um quantificador sobre $x$. $\forall x Dw$ não será considerada uma FBF porque `$x$' não ocorre em $Dw$, e $\forall x \exists x Dx$ não será considerada uma FBF porque $\exists x Dx$ já contém um quantificador sobre $x$.
...
\item Se \script{A} é uma FBF, \script{x} é uma variável, \script{A} contém pelo menos uma ocorrência de \script{x}, e \script{A} não contém quantificadores sobre \script{x}, então $\forall\script{x}\script{A}$ é uma FBF.
\item Se \script{A} é uma FBF, \script{x} é uma variável, \script{A} contém pelo menos uma ocorrência de \script{x}, e \script{A} não contém quantificadores sobre \script{x}, então $\exists\script{x}\script{A}$ é uma FBF.
\item Todas e somente as FBFs de LQ podem ser geradas por aplicações dessas regras.
\end {enumerate}

Note que o `\script{x}' que aparece na definição acima não é a variável $x$. É uma \emph{metavariável} que representa qualquer variável de LQ. Assim, $\forall xAx$ é uma FBF, mas também o são $\forall yAy$, $\forall zAz$, $\forall x_4Ax_4$ e $\forall z_9Az_9$.
...
Uma \define{sentença} é algo que pode ser verdadeiro ou falso. Em LS, toda FBF era uma sentença. Isso não será o caso em LQ. Considere a seguinte chave de simbolização:
\begin{ekey}
\item[UD:] pessoas
\item[Lxy:] $x$ ama $y$
\item[b:] Boris
\end{ekey}
Considere a expressão $Lzz$. Ela é uma fórmula atômica: um predicado binário seguido de dois termos. Toda fórmula atômica é uma FBF, então $Lzz$ é uma FBF. Ela significa alguma coisa? Você pode pensar que significa que $z$ ama a si mesmo, do mesmo modo que $Lbb$ significa que Boris ama a si mesmo. No entanto, $z$ é uma variável; ela não nomeia uma pessoa da maneira como uma constante nomeia. A FBF $Lzz$ não nos diz como interpretar $z$. Ela significa todo mundo? alguém qualquer? alguma pessoa? Se tivéssemos um quantificador sobre $z$, isso nos diria como interpretar $z$. Por exemplo, $\exists zLzz$ significaria que alguém ama a si mesmo.

Algumas linguagens formais tratam uma FBF como $Lzz$ como se tivesse, implicitamente, um quantificador universal na frente. Não faremos isso em LQ. Se você quer dizer que todo mundo ama a si mesmo, então precisa escrever o quantificador: $\forall zLzz$
...
Para uma discussão mais detalhada da teoria das descrições definidas de Russell, incluindo objeções a ela, veja a entrada de Peter Ludlow, ``Descriptions'', na \emph{Stanford Encyclopedia of Philosophy}, disponível em \url{https://plato.stanford.edu/entries/descriptions/}.
...
\item Um antílope é maior do que uma caixa de pão. Estou pensando em algo que não é maior do que uma caixa de pão, e que é ou um antílope ou um melão. Assim, estou pensando em um melão.
...
\item O melhor doce contém chocolate.
...
\item Jane é filha única (não tem irmãos).
